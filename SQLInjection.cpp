...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Convert SQL to lowercase for case-insensitive detection
    std::string lower_sql = sql;
    std::transform(lower_sql.begin(), lower_sql.end(), lower_sql.begin(), ::tolower);

    // Regex to match "or value=value", where value is a number or quoted string
    std::regex injection_pattern(R"(\sor\s+('.*?'|\d+)\s*=\s*\1)", std::regex::icase);

    if (std::regex_search(lower_sql, injection_pattern)) {
        std::cout << "Potential SQL Injection Detected! Query blocked:\n" << sql << std::endl;
        return false;
    }
